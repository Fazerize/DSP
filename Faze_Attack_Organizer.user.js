...
// @description  -
// @author       Fazerize
// @downloadURL  https://github.com/fazerize/DSP/raw/main/Faze_Attack_Organizer.user.js
// @updateURL    https://github.com/fazerize/DSP/raw/main/Faze_Attack_Organizer.user.js
...
function iTshort(nr, line) {
    ...
    buttonIcons.forEach(function(nome, num) {
        if(buttonNames[num] == ' | DONE' || buttonNames[num] == ' | Fake' || buttonNames[num] == ' | Getabbt') {
        html += '<button type="button" id="opt' + nr + '_' + num + '" class="btn" title="' + buttonNames[num] + '" style="color: ' + getFon(num) + '; font-size: ' + getSize() + 'px !important; background: linear-gradient(to bottom, ' + getTop(num) + ' 30%, ' + getBot(num) + ' 10%)">' + nome + '</button>';
        }
    })
    html += '</span></td>';
...
function check(name) {
    ...
    if (index !== -1) { // Überprüfen, ob das letzte Wort in der Liste der buttonNames enthalten ist
